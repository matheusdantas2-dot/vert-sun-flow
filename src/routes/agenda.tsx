...
import {
  ...
  type AtividadeInput,
} from "@/lib/atividades.api";
import { useClientesQuery } from "@/lib/clientes.api";
import { useProfilesQuery, type Profile } from "@/lib/profiles.api";
...
function Agenda() {
  ...
  const titulo = useMemo(() => {
    ...
    if (modo === "semana") {
      const ini = new Date(refDate);
      ini.setDate(refDate.getDate() - refDate.getDay());
  ...
  return (
    <div className="p-4 lg:p-6 space-y-4 max-w-[1500px] mx-auto">
      <header className="flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-2 flex-wrap">
            ...
            {(["mes", "semana", "dia"] as const).map((m) => (
              <button
                ...
                className={`px-3 py-1.5 text-sm font-semibold capitalize ${
                  modo === m ? "bg-vert-dark text-white" : "bg-card hover:bg-accent"
                }`}
              >
                {m}
              </button>
          ...
          <div className="inline-flex items-center gap-1">
            <button onClick={() => navegar(-1)} className="p-1.5 rounded-lg border border-border hover:bg-accent">
              <ChevronLeft className="h-4 w-4" />
            </button>
            <button onClick={() => navegar(0)} className="px-3 py-1.5 rounded-lg border border-border text-sm font-semibold hover:bg-accent">
              Hoje
            </button>
            <button onClick={() => navegar(1)} className="p-1.5 rounded-lg border border-border hover:bg-accent">
          ...
          <button
            onClick={() => setNovoOpen(new Date())}
            className="inline-flex items-center gap-1 px-3 py-1.5 rounded-lg bg-primary text-primary-foreground text-sm font-semibold"
          >
      ...
      {modo === "mes" && (
        <VistaMes
          refDate={refDate}
          atividadesNoDia={atividadesNoDia}
          onClickDia={(d) => { setRefDate(d); setModo("dia"); }}
          onCriar={(d) => setNovoOpen(d)}
        />
      )}
      {modo === "semana" && (
        <VistaSemana
      ...
      {modo === "dia" && (
        <VistaDia
          data={refDate}
          atividades={atividadesNoDia(refDate)}
          clientes={clientes}
          profiles={profiles}
          onUpdate={(id, patch) => updateMut.mutate({ id, patch })}
          onDelete={(id) => deleteMut.mutate(id)}
          onCriar={() => setNovoOpen(refDate)}
        />
      )}
      ...
      {novoOpen && (
        <NovaAtividadeModal
          ...
          onSalvar={(a) => {
            createMut.mutate(a, { onSuccess: () => setNovoOpen(null) });
          }}
...
type DiaCallback = (d: Date) => AtividadeRow[];

function VistaMes({
  refDate, atividadesNoDia, onClickDia, onCriar,
}: {
  refDate: Date;
  atividadesNoDia: DiaCallback;
      ...
      <div className="grid grid-cols-7 auto-rows-[110px]">
        {cells.map((d, i) => {
          ...
          return (
            <div
              key={i}
              className="border-t border-r border-border p-1.5 hover:bg-accent/30 cursor-pointer group relative"
              onClick={() => onClickDia(d)}
            >
              <div className="flex items-center justify-between mb-1">
                <span className={`text-xs font-bold w-6 h-6 flex items-center justify-center rounded-full ${isHoje ? "bg-vert-dark text-white" : "text-foreground"}`}>
                  {d.getDate()}
                </span>
                <button
                  onClick={(e) => { e.stopPropagation(); onCriar(d); }}
                  className="opacity-0 group-hover:opacity-100 text-vert hover:text-vert-dark"
                >
              ...
              <div className="space-y-0.5">
                {acts.slice(0, 3).map((a) => (
                  <div key={a.id} className={`text-[10px] px-1 py-0.5 rounded border truncate ${TIPO_COR[a.tipo]}`}>
                    {new Date(a.data).toLocaleTimeString("pt-BR", { hour: "2-digit", minute: "2-digit" })} {a.titulo}
                  </div>
                ))}
                {acts.length > 3 && (
                  <div className="text-[10px] text-muted-foreground px-1">+{acts.length - 3} mais…</div>
                )}
              </div>
            </div>
          );
    ...
    <div className="grid grid-cols-1 md:grid-cols-7 gap-2">
      {dias.map((d) => {
        ...
        return (
          <div
            key={d.toISOString()}
            className={`bg-card rounded-xl border ${isHoje ? "border-vert" : "border-border"} p-3 min-h-[260px]`}
          >
            <div className="flex items-center justify-between mb-2 pb-2 border-b border-border">
              <button onClick={() => onClickDia(d)} className="text-left">
                ...
                <div className={`text-xl font-display font-extrabold ${isHoje ? "text-vert-dark" : ""}`}>
                  {d.getDate()}
                </div>
              </button>
              <button onClick={() => onCriar(d)} className="p-1 rounded hover:bg-accent text-vert">
...
function VistaDia({
  data, atividades, clientes, profiles, onUpdate, onDelete, onCriar,
}: {
  data: Date;
  atividades: AtividadeRow[];
  clientes: { id: string; nome: string }[];
  profiles: Profile[];
  onUpdate: (id: string, patch: Partial<AtividadeInput>) => void;
    ...
    <div className="bg-card rounded-xl border border-border p-4">
      <div className="flex items-center justify-between mb-3">
        <h2 className="font-display font-bold text-lg flex items-center gap-2">
          <CalIcon className="h-5 w-5 text-vert" />{" "}
          {data.toLocaleDateString("pt-BR", { weekday: "long", day: "2-digit", month: "long" })}
        </h2>
        <button onClick={onCriar} className="text-sm font-semibold text-vert hover:underline inline-flex items-center gap-1">
            ...
            <div key={h} className="flex items-start gap-3 py-1.5 border-t border-border/50">
              ...
              <div className="flex-1 space-y-1">
                ...
                {acts.map((a) => {
                  ...
                  const resp = profiles.find((u) => u.id === a.responsavel_id);
                  const Icon = ICON[a.tipo];
                  return (
                    <div key={a.id} className={`flex items-center gap-2 px-3 py-2 rounded-lg border ${TIPO_COR[a.tipo]}`}>
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="text-[11px] opacity-80 truncate">
                          {new Date(a.data).toLocaleTimeString("pt-BR", { hour: "2-digit", minute: "2-digit" })} · {cliente?.nome ?? "—"} · {resp?.nome ?? "—"}
                        </div>
                      </div>
                      <select
                        value={a.status}
                        onChange={(e) => onUpdate(a.id, { status: e.target.value as AtividadeStatus })}
                        className="text-xs bg-white/60 rounded px-2 py-0.5 border border-current/20"
                      >
                      ...
                      <button onClick={() => onDelete(a.id)} className="opacity-60 hover:opacity-100">
...
function NovaAtividadeModal({
  dataInicial, clientes, profiles, currentUserId, onClose, onSalvar, saving,
}: {
  dataInicial: Date;
  clientes: { id: string; nome: string }[];
  profiles: Profile[];
  ...
  onClose: () => void;
  onSalvar: (a: AtividadeInput) => void;
  saving: boolean;
}) {
  ...
  useEffect(() => {
    if (!responsavelId && currentUserId) setResponsavelId(currentUserId);
        ...
        <div className="p-4 space-y-3">
          <div className="grid grid-cols-2 gap-3">
            <label className="block text-xs">
              Tipo
              <select value={tipo} onChange={(e) => setTipo(e.target.value as AtividadeTipo)} className={inp + " mt-1"}>
                {(Object.keys(TIPO_LABEL) as AtividadeTipo[]).map((t) => (
            ...
            <label className="block text-xs">
              Duração (min)
              <input type="number" value={duracao} onChange={(e) => setDuracao(+e.target.value)} className={inp + " mt-1"} />
            </label>
          </div>
          <label className="block text-xs">
            Título
          ...
          <div className="grid grid-cols-2 gap-3">
            <label className="block text-xs">
              Data
              <input type="date" value={data} onChange={(e) => setData(e.target.value)} className={inp + " mt-1"} />
            </label>
            <label className="block text-xs">
              Hora
              <input type="time" value={hora} onChange={(e) => setHora(e.target.value)} className={inp + " mt-1"} />
            </label>
          </div>
          <label className="block text-xs">
            Cliente
            <select value={clienteId} onChange={(e) => setClienteId(e.target.value)} className={inp + " mt-1"}>
          ...
          <label className="block text-xs">
            Responsável
            <select value={responsavelId} onChange={(e) => setResponsavelId(e.target.value)} className={inp + " mt-1"}>
              {profiles.map((u) => (
          ...
          <label className="block text-xs">
            ...
            <textarea
              ...
              className={inp + " mt-1 h-20 py-2"}
            />
          </label>
        </div>
        <div className="flex justify-end gap-2 p-4 border-t border-border bg-muted/30">
          <button onClick={onClose} className="px-4 py-2 rounded-lg border border-border text-sm font-semibold hover:bg-accent">
            Cancelar
          </button>
          <button
            onClick={submit}
