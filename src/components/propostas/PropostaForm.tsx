...
export function PropostaForm({ propostaId, initialClienteId }: Props) {
  ...
  const updatePropostaM = useUpdateProposta();
  const podePdf = usePode("exportar_pdf");

  const editing = !!propostaId;
  const existing = editing ? propostas.find((p) => p.id === propostaId) : undefined;

  const [selCliente, setSelCliente] = useState(initialClienteId ?? "");
  ...
  const [status, setStatus] = useState<PropostaStatus>("rascunho");
  ...
  useEffect(() => {
    ...
    setSelCliente(existing.clienteId);
    setIrradiacao(existing.irradiacao);
    setEficiencia(existing.eficiencia);
    setCobertura(existing.cobertura);
    setInflacao(existing.inflacao);
    setTaxaFin(existing.taxaFinanciamento);
    setTaxaCart(existing.taxaCartao);
    setStatus(existing.status);
    setItens(existing.itens.map((it) => ({ ...it })));
    setKitNome(existing.kitNome ?? "");
    setKitConsumo(existing.kitConsumoKwh);
    setMostrarComoKit(!!existing.mostrarComoKit);
    setObservacoes(existing.observacoes ?? "");
    setHidratado(true);
  }, [editing, existing, hidratado]);

  useEffect(() => {
    if (editing) return;
    if (!selCliente && clientes.length > 0) setSelCliente(clientes[0].id);
  }, [clientes, selCliente, editing]);
  ...
  useEffect(() => {
    if (editing) return; // não recalcula automaticamente em edição (preserva valores)
  ...
  }, [dim.potenciaKwp, produtos, editing]);
  ...
  const salvar = async (acao: "save" | "savePdf" | "preview" = "save") => {
    ...
    try {
      let nova;
      if (editing && existing) {
        await updatePropostaM.mutateAsync({
          id: existing.id,
          patch: {
        ...
        nova = await addPropostaM.mutateAsync({
          clienteId: cliente.id,
      ...
      const consultor = profileToConsultor(nova.consultorId || currentUserId);

      if (acao === "savePdf") {
        gerarPdfProposta({ proposta: nova, cliente, consultor, produtos, empresa, modo: "save" });
        notify.success(editing ? "Proposta atualizada" : "Proposta salva", `${nova.numero} — PDF baixado.`);
      ...
      if (acao === "preview") {
        ...
        notify.success(editing ? "Alterações salvas" : "Proposta salva", `${nova.numero} pronta para visualização.`);
        return;
      }
      notify.success(editing ? "Proposta atualizada" : "Proposta salva", `${nova.numero} ${editing ? "atualizada" : "criada como rascunho"}.`);
      navigate({ to: "/propostas" });
    } catch (e: unknown) {
  ...
  const visualizarPreviewSemSalvar = () => {
    ...
    const validade = new Date();
    validade.setDate(validade.getDate() + validadeDias);
    const consultor = profileToConsultor((existing?.consultorId) || currentUserId);
    const temp = {
      id: existing?.id ?? "preview",
      numero: existing?.numero ?? "PRÉVIA",
      versao: existing?.versao ?? 1,
      criadoEm: existing?.criadoEm ?? new Date().toISOString(),
      clienteId: cliente.id,
      consultorId: existing?.consultorId ?? currentUserId,
      validadeAte: existing?.validadeAte ?? validade.toISOString(),
      status,
      itens,
      irradiacao,
      eficiencia,
      ...
      <header className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h1 className="font-display text-2xl lg:text-3xl font-extrabold tracking-tight">
            {editing ? `Editar proposta ${existing?.numero ?? ""}` : "Nova Proposta"}
          </h1>
          <p className="text-sm text-muted-foreground mt-0.5">
            {editing ? "Ajuste itens, status e dimensionamento" : "Monte o sistema e simule retorno financeiro"}
          </p>
        </div>
        <div className="flex flex-wrap gap-2">
          ...
          <button onClick={() => salvar("save")} className="inline-flex items-center gap-2 px-5 py-2 rounded-lg bg-primary text-primary-foreground text-sm font-semibold">
            <Save className="h-4 w-4" /> {editing ? "Salvar alterações" : "Salvar proposta"}
          </button>
        </div>
      </header>
        ...
        <div className="lg:col-span-1 space-y-4">
          <div className="bg-card rounded-xl border border-border p-5 space-y-3">
            ...
            <div className="pt-2 border-t border-border">
              ...
              <select value={status} onChange={(e) => setStatus(e.target.value as PropostaStatus)} className={inp}>
                {STATUS_LIST.map((s) => <option key={s} value={s}>{STATUS_PROPOSTA_LABEL[s]}</option>)}
              </select>
            </div>
          </div>
          ...
          <div className="bg-card rounded-xl border border-border p-5 space-y-3">
            ...
            <label className="block text-xs">Observações
              <textarea value={observacoes} onChange={(e) => setObservacoes(e.target.value)} rows={3} className="w-full mt-1 px-3 py-2 rounded-lg bg-muted border border-transparent focus:bg-card focus:border-vert-light text-sm outline-none" />
            </label>
          </div>
        </div>
